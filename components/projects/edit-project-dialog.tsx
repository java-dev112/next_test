...
import { ProjectType, RemodelType, Project, Customer } from "@/types/project";
...
export function EditProjectDialog({
  ...
  useEffect(() => {
    if (project && open && customers.length > 0) {
      const customer = customers.find((c) => c.name === project.customer);
      setValue("customer", customer?.id || customers[0]?.id || "");
      setValue("projectName", project.name);
      setValue("projectDescription", project.description || "");
      setValue("remodelType", project.projectType);
      setValue("location", project.location || "");
      setCurrentImageUrl(project.coverPhoto || null);
      setSelectedFile(null);
    }
  }, [project, open, customers, setValue]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl w-[95vw] sm:w-full border-2 border-dashed border-purple-300 bg-white p-0 overflow-visible">
        <div className="flex flex-row items-center justify-between pb-3 border-b mb-0 px-4 sm:px-6 pt-4 pr-8">
          <DialogTitle className="text-lg font-semibold m-0">Edit Project</DialogTitle>
        </div>
        <form onSubmit={handleSubmit(onFormSubmit)} className="px-4 sm:px-6 py-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5 col-span-1 sm:col-span-2">
              ...
              >
                <div className="flex items-center">
                  <RadioGroupItem value="Fixed Price" id="edit-fixed-price" className="mr-1.5" />
                  <Label 
                ...
                <div className="flex items-center">
                  <RadioGroupItem value="Cost Plus" id="edit-cost-plus" className="mr-1.5" />
                  <Label 
                    htmlFor="edit-cost-plus" 
                    className={`font-normal cursor-pointer px-2.5 py-1 rounded-md text-sm transition-colors ${
                ...
                <div className="flex items-center">
                  <RadioGroupItem value="No Customer" id="edit-no-customer" className="mr-1.5" />
                  <Label 
                    htmlFor="edit-no-customer" 
                    className={`font-normal cursor-pointer px-2.5 py-1 rounded-md text-sm transition-colors ${
            ...
            <div className="space-y-1.5 col-span-2">
              <Label htmlFor="editCoverPhoto" className="text-sm">Upload a cover photo</Label>
              <div className="flex items-center gap-3">
                ...
                >
                  {currentImageUrl && !selectedFile ? "Change photo" : "Upload a cover photo"}
                </Button>
                <Input
                  id="editCoverPhoto"
                  type="file"
